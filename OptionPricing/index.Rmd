...
data <- getSymbols('AAPL', src = "yahoo", 
                   auto.assign = FALSE,
                   warnings = F)
data <- as.data.frame(data)[,c(1:4,6)]
colnames(data) <- c("Open","High","Low","Close","Adjust")
data <- cbind(date=as.character(rownames(data)),data)
    
n1 = mPlot(x = "date", y = c("Open","High","Low","Close","Adjust"), type = "Line", data = data)
